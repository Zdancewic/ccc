...
rule token = parse
  ...
  | lowercase (digit | character | '_')* { create_token lexbuf (fun x -> LIDENT x) }
  | uppercase (digit | character | '_')* { create_token lexbuf (fun x -> UIDENT x) }
  ...
  | '(' | ')' | '[' | ']' | ':' | "()"
  | '|' | "->" 
    { create_token lexbuf (fun x -> failwith "bogus symbol") }
